  ...
  constructor() {
    ...
    this.AlcoholEffect = new Effect(effectName, effectTurns, onDamage, function onShoot(player, result, playerDamaged) {
      ...
      if (result instanceof Alcohol) {
        ...
        playerDamaged instanceof Human && (getById('statusEffects').innerHTML +=  `<p onclick='displayAlcoholInfo("${result.name}", "${result.description}", "${result.img}")' id='alcohol${result.id}' style="font-size: 2em; margin-top: 1px; margin-bottom: 0px; cursor: pointer">${result.name}</p>`)
      }
      else if (result) {
        msg = "Double Live"
